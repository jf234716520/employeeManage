...
(function(){
	var modules = {
		...
		idiom:{
            js:'../../../ext/nx/pages/system/idiom/idiom.js'
        }
	};
	...
	function loadSingle(name, callback){
		...
		var cssStatus = (easyloader_apcd.css && module['css']) ? 'loading' : 'loaded';
		
		if (easyloader_apcd.css && module['css']){
			if (/^http/i.test(module['css'])){
				var url = module['css'];
			} else {
				var url = easyloader_apcd.base + 'themes/' + easyloader_apcd.theme + '/' + module['css'];
			}
		...
		if (/^http/i.test(module['js'])){
			...
			var url = easyloader_apcd.base + 'plugins/' + module['js'];
		}
		loadJs(url, function(){
			jsStatus = 'loaded';
			if (jsStatus == 'loaded' && cssStatus == 'loaded'){
		...
		function finish(){
			queues[name] = 'loaded';
			easyloader_apcd.onProgress(name);
			if (callback){
				callback();
			}
		}
		...
		function loadMm(){
			if (mm.length){
				...
				if (!queues[m]){
					...
					if (time < easyloader_apcd.timeout){
						time += 10;
						setTimeout(arguments.callee, 10);
					}
				}
			} else {
				if (easyloader_apcd.locale && doLoad == true && locales[easyloader_apcd.locale]){
					var url = easyloader_apcd.base + 'locale/' + locales[easyloader_apcd.locale];
					runJs(url, function(){
						finish();
					});
				} else {
		...
		load: function(name, callback){
			if (/\.css$/i.test(name)){
				if (/^http/i.test(name)){
					...
					loadCss(easyloader_apcd.base + name, callback);
				}
			} else if (/\.js$/i.test(name)){
				if (/^http/i.test(name)){
					loadJs(name, callback);
				} else {
					loadJs(easyloader_apcd.base + name, callback);
				}
			} else {
	...
	for(var i=0; i<scripts.length; i++){
		...
		if (m){
			easyloader_apcd.base = src.substring(0, m.index);
		}
	}

	window.using = easyloader_apcd.load;
	
	/*
	if (window.jQuery){
		jQuery(function(){
			easyloader_apcd.load('parser', function(){
				jQuery.parser.parse();
			});
		});
	}*/
